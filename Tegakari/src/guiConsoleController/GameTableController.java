...
public class GameTableController implements ActionListener, KeyListener
{
    ...
    private boolean helperAction(String cmd)
    {
        ...
        {
            out = helperAction2(cmd);
        }
        return out;
    }
    
    private boolean helperAction2(String cmd)
    {
        boolean out = true;
        // if notepad
    ...
    private void actionHelper(int index)
    {
        //IF any moadl dialogues are open do not continue
        if (!checkModals())
        {
            view.action2Enable(false);
            view.action1Enable(false);
            view.endTurnEnable(true);
            state.setChosenActionCard(selfPlayer.getActionCards().get(index));
            engine.beginTurn();
        }
    }
    ...
    private boolean checkModals()
    {
        ...
        if (windows != null) 
        {
            // FOR each window in the window array
            for (Window window : windows) 
            {
                // IF that window is a modal window, to exclude notepad
                if (window.isShowing() && window instanceof 
                        JDialog && ((JDialog)window).isModal())
                {
                    return true;
                }
            }
